...
class CurrencyConverter:
    ...
    def fetch_rate(self):
        ...
        if self.destination_currency not in data["rates"]:
            raise ValueError(f"Destination currency {self.destination_currency} not available.")

        self.exchange_rate = data["rates"][self.destination_currency]
...
def main():
    ...
    try:
        ...
        print(f"{amount} {home_currency.upper()} = {converted:.2f} {destination_currency.upper()}")
